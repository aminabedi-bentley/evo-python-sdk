...
> 4- Can one **generic reference resolver** turn friendly Python values into wire-correct
> payloads for *any* task, driven purely off the schema annotations?
> *(walked through in [`demo.ipynb`](./demo.ipynb) §7.5 "reference resolution — friendly
> values → wire payload")*
...
poc-engine-e2e/
  poc_compute_engine/                  # THE ENGINE PACKAGE
    ...
    resolver.py        # ReferenceResolver: schema-driven, generic friendly-value → wire-payload resolution
  ...
  test_resolver.py     # offline resolver tests (fake loader, real schemas) -> 10/10 pass
...
The synthesised `run(...)` signature is **reference-aware**, so it advertises *exactly*
what each parameter accepts rather than a bare `str`/`dict`: a geoscience-object
reference is typed `ObjectInput` (`str | ObjectHandle | LoadedObject` — plus SDK-typed
objects at runtime), a file reference `FileInput`, a read attribute `AttributeInput`,
and a `target` attribute slot `TargetAttrInput` (`str | CreateAttr | UpdateAttr`).
`source`/`target` object groups become per-task `TypedDict`s in the generated `.pyi`
(e.g. `NormalScoreGcpSourceInput = {object: ObjectInput, attribute: AttributeInput}`), so
IDE autocomplete and the type checker show the accepted shapes. These aliases are defined
in `resolver.py` (the same union the resolver actually accepts) and reused by both the
runtime signature (`engine._py_annotation`) and the stub generator.
...
```bash
cd poc-engine-e2e
uv sync                                       # workspace deps (evo.common / evo.oauth / evo.compute / evo.objects, ...)
uv run jupyter lab demo.ipynb                 # real OAuth, live discovery, real reference resolution, live-vs-stub breadth
```
...
uv run python generate_stubs.py               # -> wrote __init__.pyi (3 tasks)

# 2) Exercise the generic resolver against the real schemas (fake loader)
uv run python test_resolver.py                # -> 10/10 passed

# 3) Static DX from the generated stub
uv run pyright typed_usage_ok.py     # -> 0 errors
uv run pyright typed_usage_bad.py    # -> 8 errors (incl. a live-but-not-stubbed task + an override-typed param)
#   (mypy agrees: uv run mypy typed_usage_ok.py)
```
...
* **Reference resolution is real and generic.** `ReferenceResolver` (`resolver.py`) reads
  each task's schema annotations — the closed vocabulary `reference_to`,
  `supported_schemas`, `attribute_from`, `attribute_path`, `target`, `discriminator` — and
  turns friendly Python values into the wire payload for *any* task, with no per-task code.
  Object loading (needed to read a `schema_id` before picking an attribute's JMESPath) is
  behind an injectable `ObjectLoader`: the real SDK-backed loader by default, a fake in
  the offline tests. The only remaining stand-ins are `_load_object`/`_load_file` on the
  *result* side (they fabricate demo data instead of downloading the returned
  object/file); actually *running* a task still needs real
  object/attribute references (see the SDK's full kriging code-sample).
* Result-side hydration now covers **every RFC 168 output type** generically:
  `output: geoscience-object` (`get_object`/`to_dataframe`), `output: file` (`get_file`),
  and `output: attribute` (`get_attribute`), plus arrays of output objects and nullable
  outputs — so the two tasks the audit flagged for `output:file` no longer need a bespoke
  result type. The only remaining stand-ins are `_load_object`/`_load_file` themselves
  (they fabricate demo data instead of downloading); actually *running* a task still needs
  real object/attribute references (see the SDK's full kriging code-sample).
