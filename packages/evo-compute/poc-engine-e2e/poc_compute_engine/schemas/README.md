...
The bundled snapshots (`geostatistics/kriging-gcp`, `geostatistics/declustering`,
`geostatistics/normal-score-gcp`) were captured from
[`seequent/core-compute-tasks`](https://github.com/seequent/core-compute-tasks)
`compute-tasks/geostatistics/<task>/schema.json` at **v0.5.3**. `kriging-gcp` intentionally
exercises the full closed annotation vocabulary (`reference_to`, `supported_schemas`,
`attribute_from`, `attribute_path`, `target`, `discriminator`), so it doubles as the
`ReferenceResolver`'s primary end-to-end fixture.
