...
def _input_annotation(prop: dict, base: str, groups: list[str]) -> str:
    """The *accepted-input* annotation for a parameter (mirrors ``engine._py_annotation``).

    Reference-bearing params advertise the friendly union they accept; object groups
    (``source``/``target``) become a per-task ``TypedDict`` whose fields carry the same
    reference-aware types, so the ``.pyi`` shows exactly what ``run(...)`` accepts.
    """
    if "enum" in prop:
        return _annotation(prop)
    ...
    if prop.get("properties"):
        name = base + "Input"
        _render_input_group(name, prop, base, groups)
        return name
    return _annotation(prop)


def _render_input_group(name: str, node: dict, base: str, groups: list[str]) -> None:
    # total=False: run(...) validates required params at call time, so every field is
    # optional at the type level (keeps the TypedDict permissive without NotRequired).
    fields: list[str] = []
    for fname, fspec in node.get("properties", {}).items():
        ann = _input_annotation(fspec, base + _cap(fname), groups) if isinstance(fspec, dict) else "Any"
        fields.append(f"    {fname}: {ann}")
    if any(g.startswith(f"class {name}(") for g in groups):
        return  # already defined (same group shape reused)
    groups.append(f"class {name}(TypedDict, total=False):\n" + "\n".join(fields or ["    pass"]))
...
def _render_task_proxy(spec: dict, result_cls: str, input_groups: list[str]) -> str:
    schema = spec["parameters"]
    props: dict = schema.get("properties", {})
    required = list(schema.get("required", []))
    optional = [n for n in props if n not in required]
    task_base = _cap(spec["name"])
    lines = [f"class {_task_class(spec['topic'], spec['name'])}:", "    async def run("]
    lines.append("        self,")
    lines.append("        *,")
    for name in required:
        ann = _input_annotation(props[name], task_base + _cap(name), input_groups)
    ...
    for topic in sorted(by_topic):
        ...
        for spec in topic_specs:
            ...
            if override is not None:
                ...
                task_proxies.append(_render_task_proxy(spec, result_cls, input_groups))
